...
def calculate_netzbezug_analysis(df):
    ...
    summary = df_copy.groupby(['datum', 'zeitintervall'])['netzbezug'].sum().reset_index()
    
    pivot_table = summary.pivot(index='datum', columns='zeitintervall', values='netzbezug').fillna(0)
